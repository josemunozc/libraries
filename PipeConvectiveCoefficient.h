...
class ConvectiveCoefficient
{
 public:
  ConvectiveCoefficient(double radius, double flow, double EG/*, double T*/);/*
									       ...
									       solution, the mean temperature of the solution. This defines some physical properties
									       ...
									       print_data, that print all the information to the screen, are the only available from
									       outside this class.
									     */					 
  void   print_data();
 ...
 private:  /*
	     First we calculate the physical properties of the aqueous ethylene-glycol solution. The
	     following functions do the job. They are accessed from get_convective_coefficient() and
	     modify directly private variables on the class, so there is no need for a return argument.
	   */
  ...
  /*
    The next set of variables are those which are going to be calculated
    in the object based on the arguments passed to the constructor. They
    are initialized to zero in the constructor. This is in case print is
    called before get_convective_coefficient is.
  */
...
/*
  This is the constructor of the class. It receives four arguments: the external radius of the
  pipe, the volumetric flow going through the pipes, the percentage of ethylene-glycol present
  on the solution and its mean temperature.
 */
ConvectiveCoefficient::ConvectiveCoefficient (double radius, double flow, double EG/*, double T*/):
...
/* 
   The calculus of the thermal conductivity for an aqueous ethylene-glycol solution is based on 
   a product guide document from MEGlobal. It is given by:							 
   k_eg = A + B*T + C*T*T								 
   where the coefficients A,B and C depend on the percentage of ethylene glycol present in the 
   solution and  T is its temperature. The result is in Btu(F)/hr(ft2)F and needs to be 
...
/* 
   The calculus of the thermal specific heat capacity for an aqueous ethylene-glycol solution
   is based on a product guide document from MEGlobal. It is given by:							 
   Cp_eg = a + b*T + c*T*T								 
   where the coefficients A,B and C depend on the percentage of ethylene glycol present in the 
   solution and  T is its temperature. The result is in Btu/lbF and needs to be converted to
   SI units:
...
/* The calculus of the dynamic viscosity for an aqueous ethylene-glycol solution is based on 
   a product guide document from MEGlobal. It is given by:							 
   \mu_eg = 10^(a-b/(x+c))								 
   where the coefficients A,B and C depend on the temperature of the solution and x is the 
   percentage of ethylene-glycol present in the solution. The result is in centipoise and
...
/* The calculus of the specific gravity for an aqueous ethylene-glycol solution is based on 
   a product guide document from MEGlobal. It is given by:							 
   \rho_eg = a+b*x+c*T*T								 
   where the coefficients A,B and C depend on the temperature of the solution and x is the 
   percentage of ethylene-glycol present in the solution.
   This give us the specific gravity that is the ratio of the density of the solution to the
...
void ConvectiveCoefficient::ethylene_glycol_density( ) 
{
  ...
  */
  ...
  rho_eg = 998.8025*(a+b*ethylene_glycol+c*pow(ethylene_glycol,2)); /* The specific gravity is multiplied by the density of water at 60F (15.55C)
								       in kg/m3									   
								    */
}
/*
  This function does the calculus of nondimensional numbers: Reynolds
  ...
  turbulent ones. However, Roberson mention that altough it is possible
...
{
  reynolds = rho_eg*volumetric_flow*(pipe_internal_diameter)/(1000*mu_eg*numbers::PI*pow(pipe_internal_diameter/2,2));
  prandtl  = mu_eg * Cp_eg / k_eg;

  if (reynolds < 2000.) // laminar region
    f = 64./reynolds;
  if (reynolds >= 2000. && reynolds <= 4000.) // transitional region
    f = (((64./2000)-(pow(0.79*log(4000)-1.64,-2)))/(2000.-4000.))*(reynolds-2000.) + (64./2000);
  if (reynolds >  4000. && reynolds <= 100000.) // turbulent region (up to Re=100000)
    f = pow(0.79*log(reynolds)-1.64,-2);
  
  nusselt = (f/8.)*(reynolds-1000.)*prandtl/(1.+12.7*pow(f/8.,0.5)*(pow(prandtl,2./3.)-1.)); 
}
