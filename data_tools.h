		...
		std::ostream &outFile)
{
	...
	/*
	 * If we call the function with 0 as number of lines to print,
	 * we will print them all (not might not be really straightforward
	 * to understand).
	 *
	 * We are assuming 6 entries in each line of date and time container,
	 * 3 for date and 3 for time. Let's check for this if
	 * ((date_and_time[0].size() != 6))
	 * throw 4;
	 * */
	if (lines_to_print == 0)
	...
	for (unsigned int i=0; i<(unsigned int)lines_to_print; i++)
	{
		if (i==0)
			...
			outFile
			<< std::setw(2) << std::setfill('0') << date_and_time[i-1][0] << "/"
		...
		for (unsigned int j = 0; j<data[0].size(); j++)
		{
			if(fabs(data[i][j])<0.001)
				...
				outFile
				<< "\t" << std::setw(7) << std::setfill(' ')
				<< std::setprecision(3) << std::fixed << data[i][j];
		}
		outFile << "\n";
	}
}
		...
		bool day_number_column = true)
{
   ...
   * data and date data from each set of files and
   * then loop through each set of files, reading
   * them, storing the entries in temporal vectors
   * and storing those vectors in the containers
   */
	data.clear();
	date_and_time.clear();
	for (unsigned int j=0; j<filenames.size(); j++)
	{
		if (file_exists(filenames[j])==false)
		{
			std::cout << "Error file: " << filenames[j] << " does not exists!"
					<< std::endl;
			throw -1;
		}
...
time_t mktimeUTC(struct tm* timeinfo)
{
	 ...
	 *
	 * Edit: For a UTC version of mktime or your GetDate, try the following
	 * (untested):
	 * Call getenv to save the current value of the TZ environment variable
	 * (if any).
...
{
	/*
	 * It is assumed that "table" is sorted by .first
	 * INFINITY is defined in math.h in the glibc implementation
	 * */
	const double INF = 1.e100;
	if (x > table.back().first)
	{
		std::cout << "Warning. In data_tools.h -> "
				<< "interpolate_data. value out of bound." << std::endl;
		return (INF);
	}
	if (x < table[0].first)
	{
		...
		double optional_variable_value = 0.)
{
	 ...
	 */
	  ...
	  * date     time     file1_entry     file2_entry     difference  \\
	  *    normalized difference     day     month     year
	  */
	 ...
	 outFile << "a =" << alpha << std::endl
			 << "b =" << beta  << std::endl
			 << "c =" << gamma << std::endl
			 << "set title " << "\"" << main_variable_name
			 << " -  Experimental vs Predicted";

	 if (optional_variable_value != 0.)
		 outFile << "\\n" << optional_variable_name << " "
		 << optional_variable_value << " " << optional_variable_units;
	 outFile << "\"" << std::endl
			 << "set xlabel \"Experimental data " << main_variable_units
			 << "\"" << std::endl
			 << "set ylabel \"Predicted data "   << main_variable_units
			 << "\"" << std::endl
    ...
    Scatter plot
	  */
	  outFile << "set terminal pngcairo dashed size 1700,1000 font \"Arial, 18\"\n"
			  << "set output \"" << graph_filenames[0] << "\"\n";
	  outFile << "set xrange [" << range_min << ":" << range_max << "]\n"
			  << "set yrange [" << range_min << ":" << range_max << "]\n"
			  << "set xtics " << tics << "\n"
			  << "set ytics " << tics << "\n"
			  << "set grid"           << "\n"
			  << "set key vert at " << range_max + tics/5 << "," << range_max
			  << " left Left" << "\n"
			  << "set label 1 \"********************\\n RMSD = " << RMSD
			  << main_variable_units << "\\n NRMSD = " << NRMSD << "\\n R2    = " << R2
			  << " \\n********************\" at " << range_max + tics/5 << ","
			  << 0.5*(range_max+range_min) - tics << std::endl
			  << "set lmargin 0" << std::endl
			  << std::endl;
	  outFile << "plot\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"9.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt  9 pt  9 t \"September\",\\" << std::endl
			  << "\"< awk '{if(($9 == \\\"10.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 10 pt 10 t \"October  \",\\" << std::endl
			  << "\"< awk '{if(($9 == \\\"11.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 11 pt 11 t \"November \",\\" << std::endl
			  << "\"< awk '{if(($9 == \\\"12.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 12 pt 12 t \"December \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"1.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 13 pt 13 t \"January  \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"2.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 14 pt 14 t \"February \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"3.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 15 pt 15 t \"March    \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"4.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 16 pt 16 t \"April    \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"5.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 17 pt 17 t \"May      \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"6.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 18 pt 18 t \"June     \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"7.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 19 pt 19 t \"July     \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"8.000\\\")) print}' " << data_output_filename
			  << "\" u 3:4 with points lt 20 pt 20 t \"August   \",\\" << std::endl
	  ...
	  outFile << "set terminal pngcairo size 1300,1000 font \"Arial, 18\""
			  << std::endl << "set output \"" << graph_filenames[1] << "\""
			  << std::endl;

	  outFile << "set title " << "\"" << main_variable_name << " - Residuals";
	  if (optional_variable_value != 0.)
		  outFile << "\\n" << optional_variable_name << " " << optional_variable_value
		  << " " << optional_variable_units;
	  outFile << "\"" << std::endl;
	  ...
	  outFile << "plot\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"9.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt  9 pt  9 t \"September\",\\" << std::endl
			  << "\"< awk '{if(($9 == \\\"10.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 10 pt 10 t \"October  \",\\" << std::endl
			  << "\"< awk '{if(($9 == \\\"11.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 11 pt 11 t \"November \",\\" << std::endl
			  << "\"< awk '{if(($9 == \\\"12.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 12 pt 12 t \"December \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"1.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 13 pt 13 t \"January  \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"2.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 14 pt 14 t \"February \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"3.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 15 pt 15 t \"March    \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"4.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 16 pt 16 t \"April    \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"5.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 17 pt 17 t \"May      \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"6.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 18 pt 18 t \"June     \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"7.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 19 pt 19 t \"July     \",\\" << std::endl
			  << "\"< awk '{if(($9 ==  \\\"8.000\\\")) print}' " << data_output_filename
			  << "\" u 4:5 with points lt 20 pt 20 t \"August   \"\\" << std::endl
			  << std::endl
			  << std::endl;
	  /*
	   * Plot time series
	  ...
	  outFile << "plot\\" << std::endl
			  << "\"" << data_output_filename << "\" u 1:4 w lp lt 25 t \"Predicted   \",\\"
			  << std::endl
			  << "\"" << data_output_filename << "\" u 1:3 w lp lt 1  t \"Experimental\"\\"
			  << std::endl;
...
Names::Names ()
{
  ...
  */  
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_05_09.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_05_10.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_05_11.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_05_12.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_01.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_02.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_03.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_04.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_05.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_06.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_07.txt");
  bh_A_temperatures
  .push_back ("/home/zerpiko/input/trl_borehole_data/borehole_A_average_data/average_borehole_A_06_08.txt");
  /*
    Metereological data from Met Office
  */
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2005_09.txt"); // 0
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2005_10_using_2006_data.txt"); // 1
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2005_11.txt"); // 2
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2005_12.txt"); // 3
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_01.txt"); // 4
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_02.txt"); // 5
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_03.txt"); // 6
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_04.txt"); // 7
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_05.txt"); // 8
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_06.txt"); // 9
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_07.txt"); // 10
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_08.txt"); // 11
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_08_up_to_day_22.txt"); // 12
  met_data_met_office.push_back ("/home/zerpiko/input/met_data/met_office/hourly_average_met_office_data_from_23_08_2005_to_14_11_2005.txt"); // 13
  /*
    Metereological data from TRL
  */
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_05_09.txt"); // 0
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_05_10.txt"); // 1
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_05_11.txt"); // 2
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_05_12.txt"); // 3
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_06_01.txt"); // 4
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_06_02.txt"); // 5
  met_data_trl.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_03.txt"); // 6
  //<<-- replace this file since the correspoding TRL's is from 2007 and it seems that that year was 
  // particularly hot
  //met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_06_03.txt"); // 6
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_06_04.txt"); // 7
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_06_05.txt"); // 8
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_06_06.txt"); // 9
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_06_07.txt"); // 10
  met_data_trl.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_08.txt");     // 11
  met_data_trl.push_back ("/home/zerpiko/input/met_data/met_office/met_office_2006_08_up_to_day_22.txt"); // 12
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_from_23_08_2005_to_14_11_2005.txt"); // 13
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_from_15_11_2005_to_20_02_2006.txt"); // 14
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_from_21_02_2006_to_26_04_2006.txt"); // 15
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_from_27_04_2006_to_31_10_2006.txt"); // 16
  met_data_trl.push_back ("/home/zerpiko/input/met_data/trl/hourly_average_met_data_from_01_11_2006_to_28_02_2007.txt"); // 17
  /*
    DAILY averages from the BADC
  */
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1985.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1986.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1987.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1988.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1989.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1990.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1991.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1992.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1993.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1994.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1995.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1996.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1997.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1998.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_1999.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_2000.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_2001.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_2002.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_2003.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_2004.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_2005.dat");
  met_data_badc_daily_averages.push_back("/home/zerpiko/input/met_data/met_office/daily_averages_solar_air_temperature_2006.dat");
}
